 ...
 */
const merge = config => Object.assign({}, DEFAULTS, config);
 ...
 * @returns {string}
 */
const getReleasesPath = config => safeFn(config, ({ deployTo, releasesDir }) => path.join(deployTo, releasesDir));
 ...
 * @returns {string}
 */
const getCurrentPath = config => safeFn(config, ({ deployTo, currentDir }) => path.join(deployTo, currentDir) );

module.exports = { merge, getReleasesPath, getCurrentPath }; 