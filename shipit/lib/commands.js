 ...
 * @returns {boolean}
 */
const numOrStr = x => (typeOf('string', x) || typeOf('number', x));
 ...
 * @returns {boolean}
 */
const checkStr = x => (typeOf('string', x) && x.length > 0);
 ...
 * @returns {string}
 */
const trimTrailing = (c, s) => s.lastIndexOf(c) === s.length - 1 ? s.substring(0, s.length - 1) : s;
 ...
 * @throws {TypeError}
 */
const mkdir = p => safePathCmd(`mkdir -p ${String(p)}`, p);
 ...
 * @throws {TypeError}
 */
const countDirs = p => safePathCmd(`find ${trimTrailing('/', String(p))}/ -maxdepth 1 -mindepth 1 -type d | wc -l`, p);
 ...
 * @throws {TypeError}
 */
const rm = p => safePathCmd(`rm -rf ${String(p)}`, p);
 ...
 * @throws {TypeError}
 */
const takeLatest = p => safePathCmd(`ls -rd1 ${trimTrailing('/', String(p))}/* | head -n 1`, p);
 ...
 * @throws {TypeError}
 */
const gitRev = () => 'git rev-parse @';

module.exports = { mkdir, symlink, keepLastOf, revision, countDirs, rm, takeLatest, gitRev }; 