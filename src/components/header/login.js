...
class Login extends Component {
    ...
    _onMouseOver() {
        ...
        import(/* webpackChunkName: "swalauth" */ './../popups/swal-auth').then((modal) => this.setState({
            Modal: modal.default
        }));
    }
    ...
    render() {
        ...
        let classes = ['header__login', 'col-auto', 'right', 'pl-0', 'header-login'];
        if (isAuthentificated()) classes.push(['header-login--is-auth']);
        return (
            <div className={classes.join(' ')}>
                {isAuthentificated() ?
                    <Dropdown isOpen={this.state.dropdownOpen} toggle={this.toggle} size="lg" direction="left">
                        <DropdownToggle className="header-login__btn float-right btn">
                            {name ?
                                ...
                                <ReactSVG src={Icon} svgClassName="svg-icon svg-icon--header" className="header-login__icon-svg" />}
                        </DropdownToggle>
                        <DropdownMenu right>
                            ...
                            <DropdownItem className="text-muted" onClick={this._onLogoutClick}>
                                <ReactSVG src={Logout} svgClassName="svg-icon svg-icon--header" className="header-logout__svg" />
