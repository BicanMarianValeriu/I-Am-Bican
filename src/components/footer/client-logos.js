...
import { fetchDispatcher, FETCH_CLIENTS_FULFILLED } from '../../actions/actions';
...
class ClientLogos extends Component {
    ...
    componentDidUpdate() {
        ...
        var current = Array.from(Array(columns).keys());
        var lastFrame = -1;
        ...
        const _updateLogo = (num) => {
            ...
            for (let i = 0; i < counter; i++) {
                ...
                if (compStyles.getPropertyValue('opacity') > .5) {
                    if (nextImage === i) {
                        _updateLogo(num);
                        return
                    }
                ...
                <div className="container">
                    <div className="row">
                        <div className="col text-center mb-4">
                            <h3 className="text-font--cursive text-color--primary company-logos__title">Just Few Of The Clients</h3>
                        </div>
                    </div>
                    <div className="row">
                        {this.renderColumns()}
